    ...
    while (idx < config_size) {
        ...
        case TOUCH_OBJECT_N_CLASSIFICATION:
            ...
            if (obj >= touch_hcd->max_objects) {
                ...
                if (tp_debug != 0) {
                    for (num = 0; num < report_size; num++) {
                        TPD_INFO("syna data:[0x%2x]", tcm_info->report.buffer.buf[num]);
                    }
                }
                return -1;
            }
...
static int syna_report_refresh_switch(void *chip_data, int fps)
{
    ...
    if (!*tcm_info->in_suspend && !tcm_info->game_mode) {
        for (num = 0; num < tcm_info->fps_report_rate_num; num = num + 2) {
            if (fps == tcm_info->fps_report_rate_array[num]) {
                send_value = tcm_info->fps_report_rate_array[num + 1];
            }
        }
...
static void init_chip_dts(struct device *dev, void *chip_data)
{
    ...
    if (tcm_info->fps_report_rate_num > 0 && tcm_info->fps_report_rate_num <= FPS_REPORT_NUM
        ...
        rc = of_property_read_u32_array(chip_np, "fps_report_rate", num_array, tcm_info->fps_report_rate_num);
        if (rc) {
            TPD_INFO("fps_report_rate not specified %d\n", rc);
        } else {
            for (num = 0; num < tcm_info->fps_report_rate_num; num++) {
                tcm_info->fps_report_rate_array[num] = num_array[num];
                TPD_INFO("fps_report_rate is: %d\n", tcm_info->fps_report_rate_array[num]);
            }
        }
    } else {
        //default :1:120hz 2:180hz is for 19101 19191 20131
