...
#include "wireless_ic/oppo_chargepump.h"	//for WPC
#else
...
#else
			if (!val->intval && chip->wireless_support) {
				val->intval = oppo_wpc_get_status();  
			}
#endif
			break;
#ifdef CONFIG_OPPO_CHECK_CHARGERID_VOLT
		...
		char __user *buff, size_t count, loff_t *off)
{
	...
	if (chip->vooc_show_ui_soc_decimal) {
		...
		if(svooc_is_control_by_vooc != true
				&&  chip->boot_completed != false && chip->calculate_decimal_time == 0 && oppo_chg_show_vooc_logo_ornot() == true) {
...
static void oppo_chg_show_ui_soc_decimal(struct work_struct *work)
{
	...
	pr_err("[oppo_chg_ui_soc_decimal]chip->ui_soc_decimal+chip_ui_soc: %d , speed: %d, soc :%d\n ",
...
static void oppo_chg_get_battery_data(struct oppo_chg_chip *chip)
{
	...
	if (chip->chg_ops->get_charger_current && oppo_vooc_get_allow_reading() == true) {
...
static void battery_notify_gauge_i2c_err_check(struct oppo_chg_chip *chip)
{
	if (oppo_gauge_get_i2c_err() > 0) {
		chip->notify_code |= 1 << NOTIFY_GAUGE_I2C_ERR;
	}

	oppo_gauge_clear_i2c_err();
}
...
#else
        if ((chip->charger_exist || oppo_wpc_get_status()) 
            && chip->batt_exist && chip->batt_full && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)) {
#endif
		chip->sleep_tm_sec = 0;
		if (oppo_short_c_batt_is_prohibit_chg(chip)) {
...
#else
        } else if ((chip->charger_exist || oppo_wpc_get_status()) && chip->batt_exist && (CHARGING_STATUS_FAIL != chip->charging_state) && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)) {
#endif        
		chip->sleep_tm_sec = 0;
		chip->prop_status = POWER_SUPPLY_STATUS_CHARGING;
	...
	if (chip->charger_exist) {
		if (chgr_dbg_total_time != 0) {
			...
			if (!chip->total_time) {
				...
				chip->total_time = OPPO_CHG_UPDATE_INTERVAL_SEC + cur_chg_time - start_chg_time;
			}
		};
	}
...
void oppo_chg_cancel_update_work_sync(void)
{
	if (!g_charger_chip) {
		return;
	}
...
void oppo_chg_restart_update_work(void)
{
	if (!g_charger_chip) {
		return;
	}
...
int oppo_chg_get_chg_ichging(void)
{
        if (!g_charger_chip) {
                return 250;
        } else {
...
void oppo_smart_charge_by_cool_down(struct oppo_chg_chip *chip, int val)
{
	...
	if (val & SMART_COMPATIBLE_VOOC_CHARGER_CURRENT_BIT0) {
		chip->limits.input_current_vooc_ma_high = OPPO_CHG_1200_CHARGING_CURRENT;
		chip->limits.input_current_vooc_ma_warm = OPPO_CHG_1200_CHARGING_CURRENT;
		chip->limits.input_current_vooc_ma_normal = OPPO_CHG_1200_CHARGING_CURRENT;
		chip->cool_down_done = true;
	}
