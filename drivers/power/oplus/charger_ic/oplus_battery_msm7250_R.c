...
//#include "oppo_battery_msm7250.h"
...
/* Jianchao.Shi@BSP.CHG.Basic, 2016/12/20, sjc Add for charging */
		if (smblib_get_prop_dfp_mode(chg) != POWER_SUPPLY_TYPEC_NONE) {
			printk(KERN_ERR "!!!!! fake usb insertion\n");
	...
	if(g_oppo_chip->vbatt_num == 2){
		...
		if(((chg->svooc_detect_time - chg->svooc_detach_time) <= 1000) && oppo_vooc_get_detach_unexpectly()
			&& g_oppo_chip->temperature <= 440 && g_oppo_chip->temperature >= 45){
			if(g_oppo_chip->svooc_disconnect_count < 3)
				g_oppo_chip->svooc_disconnect_count += 1;
			else
				oppo_vooc_set_disable_real_fast_chg(true);
			chg_err("g_oppo_chip->svooc_disconnect_count = %d\n",g_oppo_chip->svooc_disconnect_count);
		}else{
			g_oppo_chip->svooc_disconnect_count = 0;
			oppo_vooc_set_disable_real_fast_chg(false);
		}
...
/* Jianchao.Shi@BSP.CHG.Basic, 2017/01/22, sjc Add for charging */
		...
		if (chg->fake_usb_insertion) {
			printk(KERN_ERR "!!!!! fake usb pull out\n");
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* Zhangkun@BSP.CHG.Basic, 2020/08/17, Add for svooc detect and detach */
			if(chg->svooc_detach_time > 0 && g_oppo_chip->vbatt_num == 2){
				chg->svooc_detach_time = 0;
				oppo_vooc_set_detach_unexpectly(false);
				oppo_vooc_set_disable_real_fast_chg(false);
				chg_err("chg->svooc_detach_time =  %d, detach_unexpectly = %d\n",chg->svooc_detach_time, oppo_vooc_get_detach_unexpectly());
			}
#endif
	}
