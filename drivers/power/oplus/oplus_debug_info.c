...
#define OPLUS_CHG_VOOC_INPUT_CURRENT_MAX_CNT 20
...
#define OPLUS_CHG_VOOC_STRATEGE_DETA_VOLT 3

#define OPLUS_CHG_START_STABLE_TIME (10) //单位s
...
#else
//for test
#define OPLUS_CHG_BATT_AGING_CHECK_CNT   10
#define OPLUS_CHG_BATT_AGING_CHECK_TIME  (10)
#endif
...
static int oplus_chg_chg_is_normal_path(struct oppo_chg_chip *chip);
...
static int oplus_chg_chg_batt_capacity_jump_check(struct oppo_chg_chip *chip);

#ifdef CONFIG_OPLUS_KEVENT_UPLOAD
static int oplus_chg_pack_debug_info(struct oppo_chg_chip *chip)
{
...
#if 0
static void oplus_chg_print_debug_info(struct oppo_chg_chip *chip)
{
	int ret = 0;
	int i;
...
static int oplus_chg_get_input_power(struct oppo_chg_chip *chip)
{
	int input_power;
...
static int oplus_chg_get_vooc_adapter_type_index(struct oppo_chg_chip *chip)
{
	int i;
	int arr_size = sizeof(vooc_charger_type_list) / sizeof(vooc_charger_type_list[0]);
...
static int oplus_chg_get_vooc_adapter_is_low_input_power(struct oppo_chg_chip *chip)
{
...
static int oplus_chg_get_vooc_chg_max_input_power(struct oppo_chg_chip *chip)
{
	int vooc_adapter_type_index = -1;
	int adapter_input_power = 0;
...
static int oplus_chg_get_vooc_input_power(struct oppo_chg_chip *chip)
{
	int i;
	int input_power = 0;
	int vol,cur;
...
static int oplus_chg_get_charge_power(struct oppo_chg_chip *chip)
{
	int charge_power;
...
static int oplus_chg_vooc_charge_strategy_init(struct oppo_chg_chip *chip)
{
	int i;
	int index;
	...
	static int input_current_index_update_flag = 0;

	if (oplus_chg_debug_info.vooc_charge_strategy != NULL)
	{
		for(i = oplus_chg_debug_info.vooc_charge_input_current_index;
				(i < OPLUS_CHG_VOOC_INPUT_CURRENT_MAX_CNT) && (oplus_chg_debug_info.vooc_charge_strategy->input_current[i].vol > 0); i++) {
			if (chip->batt_volt <= (oplus_chg_debug_info.vooc_charge_strategy->input_current[i].vol - OPLUS_CHG_VOOC_STRATEGE_DETA_VOLT)) {
				...
				if (oplus_chg_debug_info.vooc_charge_strategy->input_current[i].time > 0) {
					if (oplus_chg_debug_info.vooc_charge_cur_state_chg_time >= oplus_chg_debug_info.vooc_charge_strategy->input_current[i].time) {
						if ((i < (OPLUS_CHG_VOOC_INPUT_CURRENT_MAX_CNT - 1)) && (oplus_chg_debug_info.vooc_charge_strategy->input_current[i + 1].vol > 0)) {
							i++;
						}
					}
				}
				...
				input_current_index_update_flag = 0;

				break;
			}
			else {
				if (!input_current_index_update_flag) {
					if ((i < (OPLUS_CHG_VOOC_INPUT_CURRENT_MAX_CNT - 1)) && (oplus_chg_debug_info.vooc_charge_strategy->input_current[i + 1].vol > 0)) {
						oplus_chg_debug_info.vooc_charge_input_current_index = i + 1;
					}
					oplus_chg_debug_info.vooc_charge_cur_state_chg_time = 0;
					input_current_index_update_flag = 1;
				}
	...
	if (oplus_chg_debug_info.vooc_charge_strategy) {
		for(i = 0; (i < OPLUS_CHG_VOOC_INPUT_CURRENT_MAX_CNT) && (oplus_chg_debug_info.vooc_charge_strategy->input_current[i].vol > 0); i++) {
			if (chip->batt_volt <= (oplus_chg_debug_info.vooc_charge_strategy->input_current[i].vol - OPLUS_CHG_VOOC_STRATEGE_DETA_VOLT)) {
				oplus_chg_debug_info.vooc_charge_input_current_index = i;
				oplus_chg_debug_info.vooc_charge_cur_state_chg_time = 5;
				break;
		...
		if ((i > 0) && (oplus_chg_debug_info.vooc_charge_input_current_index == 0)) {
			i = i -1;
			oplus_chg_debug_info.vooc_charge_input_current_index = i;
			oplus_chg_debug_info.vooc_charge_cur_state_chg_time = 5;
		}
...
#if 0
static int oplus_chg_vooc_charge_strategy_post(struct oppo_chg_chip *chip)
{
...
int oplus_chg_debug_get_cooldown_current(int chg_current_by_tbatt, int chg_current_by_cooldown)
{
	...
	if (chg_current_by_tbatt > chg_current_by_cooldown) {
...
static int oplus_chg_debug_fastchg_monitor_condition(struct oppo_chg_chip *chip)
{
	if (oplus_chg_debug_info.fastchg_stop_cnt < OPLUS_CHG_DEBUG_FASTCHG_STOP_CNT) {
		oplus_chg_debug_info.fastchg_stop_cnt++;
		if (chip->icharging < -500) {
			oplus_chg_debug_info.fastchg_stop_cnt = OPLUS_CHG_DEBUG_FASTCHG_STOP_CNT;
			return 1;
		}
		return 0;
	}
...
static int oplus_chg_fastchg_is_allowed(struct oppo_chg_chip *chip)
{
	struct oppo_vooc_chip *vooc_chip = NULL;
...
static int oplus_chg_normal_chg_input_power_check(struct oppo_chg_chip *chip)
{
...
static int oplus_chg_normal_chg_consume_power_check(struct oppo_chg_chip *chip)
{
	int input_power;
	int charge_power;
...
static int oplus_chg_fastchg_chg_input_power_check(struct oppo_chg_chip *chip)
{
	int input_power;
	int charge_power;
...
static int oplus_chg_fastchg_consume_power_check(struct oppo_chg_chip *chip)
{
	int input_power;
	int charge_power;
...
static int oplus_chg_fastchg_power_limit_check(struct oppo_chg_chip *chip)
{
...
static int oplus_chg_input_power_limit_check(struct oppo_chg_chip *chip)
{
	int float_voltage;
	union power_supply_propval pval = {0, };
...
static int oplus_chg_chg_slow_check(struct oppo_chg_chip *chip)
{
	...
	if (oppo_vooc_get_fastchg_started()) {
		if (oplus_chg_debug_info.fast_chg_type == FASTCHG_CHARGER_TYPE_UNKOWN) {
			oplus_chg_debug_info.fast_chg_type = oppo_vooc_get_fast_chg_type();
...
static int oplus_chg_chg_full_check(struct oppo_chg_chip *chip)
{
	union power_supply_propval pval = {0, };
	int status, capacity;
...
//TODO
static int oplus_chg_chg_batt_aging_check(struct oppo_chg_chip *chip)
{
	union power_supply_propval pval = {0, };
	int ret;
	struct timespec ts;
...
int oplus_chg_debug_set_soc_info(struct oppo_chg_chip *chip)
{
...
int oplus_chg_debug_chg_monitor(struct oppo_chg_chip *chip)
{
	int rc = 0;
