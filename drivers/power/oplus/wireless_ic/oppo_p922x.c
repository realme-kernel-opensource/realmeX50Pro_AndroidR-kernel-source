...
extern void oppo_dcin_irq_enable(bool enable);
...
static void wpc_battery_update(void)
{
	...
	power_supply_changed(g_oppo_chip->batt_psy);
#else
	power_supply_changed(&g_oppo_chip->batt_psy);
#endif
}
...
static void p922x_clear_irq(struct oppo_p922x_ic *chip, char mark0, char mark1)
{
...
static void p922x_set_FOD_parameter(struct oppo_p922x_ic *chip, char parameter)
{
...
static int p922x_set_tx_Q_value(struct oppo_p922x_ic *chip)
{
...
static int p922x_set_tx_cep_timeout(struct oppo_p922x_ic *chip)
{
...
static int p922x_set_tx_led_brightness(struct oppo_p922x_ic *chip)
{
	int pwm_pulse;
...
static int p922x_decrease_vout_to_target(struct oppo_p922x_ic *chip, int vout_target, int vout_min)
{
	int temp_value;
...
static int p922x_increase_vout_to_target(struct oppo_p922x_ic *chip, int vout_target, int vout_max)
{
	int temp_value;
...
static void p922x_charger_init(struct oppo_p922x_ic *chip)
{
...
static void p922x_self_reset(struct oppo_p922x_ic *chip, bool to_BPP)
{
...
#if 0
static int p922x_get_vrect_iout_offline(struct oppo_p922x_ic * chip)
{
...
static int p922x_get_vout(struct oppo_p922x_ic * chip)
{
	char val_buf[5] = {0,0,0,0,0};
	int vout_value = 0;
...
static int p922x_get_special_ID(struct oppo_p922x_ic *chip)
{
	int rc;
	char temp[2];
...
void p922x_restart_charger(struct oppo_p922x_ic *chip)
{
...
static void p922x_ready_to_switch_to_charger(struct oppo_p922x_ic *chip, bool reset_ceptimeout)
{
	if (reset_ceptimeout) {
...
static void p922x_ready_to_switch_to_chargepump(struct oppo_p922x_ic *chip)
{
...
#ifdef FASTCHG_TEST_BY_TIME
static int p922x_fastcharge_test_40w(struct oppo_p922x_ic *chip)
{
...
static void p922x_fastcharge_current_adjust_40w(struct oppo_p922x_ic *chip, bool is_init)
{
	...
	if (g_oppo_chip->temperature < g_oppo_chip->limits.little_cool_bat_decidegc || g_oppo_chip->temperature > g_oppo_chip->limits.warm_bat_decidegc) {
	...
	if (g_oppo_chip->batt_volt >= 4370
		&& ((g_oppo_chip->icharging < 0) && ((-1 * g_oppo_chip->icharging) < 800))
	...
	if (chip->p922x_chg_status.wpc_reach_stable_charge) {
		if (!chip->p922x_chg_status.wpc_reach_4370mv && (g_oppo_chip->batt_volt >= 4370)) {
			...
			goto ADJUST_FINISH;
		}/* else if (chip->p922x_chg_status.wpc_reach_4370mv && (g_oppo_chip->batt_volt >= 4370)) {
		}*/
	}
...
static void p922x_TX_message_process(struct oppo_p922x_ic *chip)
{
...
static int p922x_resume_chargepump_fastchg(struct oppo_p922x_ic *chip)
{	
	if ((chip->p922x_chg_status.wpc_reach_4370mv == false)
		&& (g_oppo_chip->batt_volt <= 4370)
		&& (g_oppo_chip->temperature < 390)
 		&& g_oppo_chip->temperature > g_oppo_chip->limits.little_cool_bat_decidegc) {
 		chip->p922x_chg_status.iout_stated_current = fasctchg_current[chip->p922x_chg_status.fastcharge_level];
...
int oppo_wpc_chg_parse_chg_dt(struct oppo_p922x_ic *chip)
{
	int rc;
...
bool oppo_wpc_get_epp_chg_allow(struct oppo_p922x_ic *chip)
{
	int temp = 0;
...
bool oppo_wpc_get_fastchg_allow(struct oppo_p922x_ic *chip)
{
	int temp = 0;
	int cap = 0;
...
int p922x_charge_set_max_current_by_tbatt(struct oppo_p922x_ic *chip)
{
	int charging_current = 512;
	static int pre_tbatt = BATTERY_STATUS__NORMAL;

	int now_tbatt = oppo_chg_get_tbatt_status();
	...
	if(pre_tbatt != now_tbatt){
		if(pre_tbatt == BATTERY_STATUS__REMOVED
			...
			if ((g_oppo_chip->temperature > g_oppo_chip->limits.little_cold_bat_decidegc)
				&& (g_oppo_chip->temperature < g_oppo_chip->limits.warm_bat_decidegc)) {
				chip->p922x_chg_status.adapter_type = ADAPTER_TYPE_UNKNOW;
			}
		}
		pre_tbatt = now_tbatt;
...
int oppo_wpc_set_input_current(struct oppo_p922x_ic *chip)
{
	int current_limit = 0;
	...
	if (!chip) {
		chg_err("oppo_p922x_ic is not ready\n");
		return -1;
	}
	...
	if(g_oppo_chip->batt_full == true && g_oppo_chip->in_rechging == false){
		chip->p922x_chg_status.wpc_chg_param.pre_input_ma = 0;
		current_limit = 200;
	}
	...
	if(current_limit !=	chip->p922x_chg_status.wpc_chg_param.pre_input_ma){
		...
		oppo_chg_set_input_current_without_aicl(current_limit);
	}
	return 0;
}

int p922x_charge_set_max_current_by_adapter_power(struct oppo_p922x_ic *chip)
{
...
int p922x_charge_set_max_current_by_led(struct oppo_p922x_ic *chip)
{
	return 0;
}

int p922x_charge_set_max_current_by_cool_down(struct oppo_p922x_ic *chip)
{
	oppo_chg_get_cool_down_status();
...
static int p922x_charge_status_process(struct oppo_p922x_ic *chip)
{
...
#ifndef FASTCHG_TEST_BY_TIME
	if ((g_oppo_chip->temperature < g_oppo_chip->limits.cold_bat_decidegc) || (g_oppo_chip->temperature > g_oppo_chip->limits.hot_bat_decidegc)) {
	...
	case WPC_CHG_STATUS_READY_FOR_EPP:
		...
		if (chip->p922x_chg_status.epp_current_limit >= chip->p922x_chg_status.wpc_chg_param.epp_input_ma
				|| (chip->p922x_chg_status.epp_current_limit >= chip->p922x_chg_status.wpc_chg_param.epp_temp_warm_input_ma
					&& oppo_chg_get_tbatt_status() == BATTERY_STATUS__WARM_TEMP)) {
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE_FOR_CHARGEPUMP:
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			temp_value = p922x_increase_vout_to_target(chip, g_oppo_chip->batt_volt * 4 + 300, WPC_CHARGE_VOLTAGE_CHGPUMP_MAX);
			if (temp_value != 0) {
				p922x_reset_CEP_flag(chip);
			} else {
	...
	case WPC_CHG_STATUS_CHARGEPUMP_INIT:
		...
		if (chip->p922x_chg_status.vout < (g_oppo_chip->batt_volt * 4 + 300)) {
		...
		if (chargepump_check_dwp_status() == -1) {
			...
			if ((chip->p922x_chg_status.vout >= 19000)
				|| (chip->p922x_chg_status.vout >= (g_oppo_chip->batt_volt * 4 + 600))) {
			...
			batt_volt = oppo_gauge_get_batt_mvolts();
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHARGER:
		...
		if (chip->p922x_chg_status.adapter_type == ADAPTER_TYPE_VOOC) {
			if (g_oppo_chip->icharging > -800 && oppo_gauge_get_batt_current() > -800) {
				self_reset_cnt++;
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_FOR_RESTART:
		...
		temp_value = p922x_decrease_vout_to_target(chip, g_oppo_chip->batt_volt * 4 + 300, WPC_CHARGE_VOLTAGE_CHGPUMP_MIN);
		if (temp_value == 0) {
...
static void p922x_get_running_mode(struct oppo_p922x_ic *chip)
{
	int rc = -1;
	char temp;
...
/* Ping.Zhang@BSP.TP.Init, 2020/03/02, Add for notify touchpanel status */
		...
		if (p922x_chip->p922x_chg_status.charge_online == true) {			
			p922x_chip->p922x_chg_status.charge_online = false;
			if (g_oppo_chip->charger_type == POWER_SUPPLY_TYPE_WIRELESS)
...
static void p922x_task_work_process(struct work_struct *work)
{
	...
	if (p922x_chip->p922x_chg_status.charge_online) {
		...
		oppo_wpc_set_input_current(chip);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oppo_p922x_ic *chip = container_of(dwork, struct oppo_p922x_ic, p922x_self_reset_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oppo_p922x_ic *chip = container_of(dwork, struct oppo_p922x_ic, p922x_test_work);
...
static int p922x_driver_probe(struct i2c_client *client, const struct i2c_device_id *id) 
{
	...
	oppo_wpc_chg_parse_chg_dt(chip);
	...
	if (g_oppo_chip && !g_oppo_chip->charger_exist)
		p922x_idt_connect_shedule_work();
